...
def read_in_chunks(fp, start_chunk, end_chunk, small_chunk_size=1000000):
    ...
    while end_chunk >= num * small_chunk_size:
        print('Size processed:- ', num * small_chunk_size)
        print('Before file pointer', fp.tell())
        data = fp.read(small_chunk_size)
        print('current file pointer:- ', fp.tell())
        if not data:
            break
        num += 1
        #end_chunk -= end_chunk
        yield data
    print('Breaking:- ', end_chunk - (num - 1) * small_chunk_size)
